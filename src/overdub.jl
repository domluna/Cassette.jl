...
if VERSION < v"1.2.0-DEV.573"
    function specialize_method(method, metharg, methsp, world, force)
        return Core.Compiler.code_for_method(method, metharg, methsp, world, force)
    end
else
    function specialize_method(method, metharg, methsp, world, force)
        return Core.Compiler.specialize_method(method, metharg, methsp, force)
    end
end
...
function reflect(@nospecialize(sigtypes::Tuple), world::UInt=typemax(UInt))
    ...
    S = Tuple{map(s -> Core.Compiler.has_free_typevars(s) ? typeof(s.parameters[1]) : s,
                  sigtypes)...}
    ...
    method_instance = specialize_method(method, type_signature, raw_static_params, world,
                                        false)
...
function overdub_pass!(reflection::Reflection, context_type::DataType,
    ...
    code_info.slotnames = Any[:overdub, OVERDUB_CONTEXT_NAME, OVERDUB_ARGUMENTS_NAME,
                              code_info.slotnames...]
    ...
    if method.isva
        ...
        if hastagging(context_type)
            trailing_arguments = Expr(:call, GlobalRef(Cassette, :_tagged_new_tuple_unsafe),
                                      overdub_ctx_slot)
        else
        ...
        for i in n_method_args:n_actual_args
            push!(overdubbed_code,
                  Expr(:call, GlobalRef(Core, :getfield), overdub_args_slot, offset - 1))
            push!(overdubbed_codelocs, code_info.codelocs[1])
            push!(trailing_arguments.args, SSAValue(length(overdubbed_code)))
            offset += 1
        end
        push!(overdubbed_code,
              Expr(:(=), SlotNumber(n_method_args + n_prepended_slots), trailing_arguments))
        push!(overdubbed_codelocs, code_info.codelocs[1])
    end
    ...
    if istaggingenabled && !iskwfunc
        ...
        modules = Any[]
        original_code_region = view(overdubbed_code,
                                    original_code_start_index:length(overdubbed_code))
        ...
        for i in 1:length(modules)
            modules[i] = Expr(:call,
                              Expr(:nooverdub, GlobalRef(Cassette, :fetch_tagged_module)),
                              overdub_ctx_slot, modules[i])
        end

        # insert `fetch_tagged_module`s at the `original_code_start_index`
        insert_statements!(overdubbed_code, overdubbed_codelocs,
                           (x, i) -> i == original_code_start_index ? length(modules) + 1 :
                                     nothing, (x, i) -> [modules..., x])
                           ...
                           (x, i) -> begin
                               ...
                               return [rhs,
                                       Expr(:(=), globalref,
                                            Expr(:call,
                                                 Expr(:nooverdub,
                                                      GlobalRef(Cassette, :untag)),
                                                 SSAValue(i), overdub_ctx_slot)),
                                       Expr(:call,
                                            Expr(:nooverdub,
                                                 GlobalRef(Cassette,
                                                           :tagged_globalref_set_meta!)),
                                            overdub_ctx_slot, tagmodssa, name, SSAValue(i))]
                           end)
                               ...
                               if Base.Meta.isexpr(stmt, :replaceglobalref)
                                   ...
                                   result = Expr(:call,
                                                 Expr(:nooverdub,
                                                      GlobalRef(Cassette,
                                                                :tagged_globalref)),
                                                 overdub_ctx_slot, tagmodssa, name,
                                                 globalref)
    ...
    if istaggingenabled && !iskwfunc
        insert_statements!(overdubbed_code, overdubbed_codelocs,
                           ...
                           (x, i) -> [Expr(:call,
                                           Expr(:nooverdub, GlobalRef(Cassette, :untag)),
                                           x.args[1], overdub_ctx_slot),
    ...
    if istaggingenabled && !iskwfunc
        replace_match!(x -> Base.Meta.isexpr(x, :new) || Base.Meta.isexpr(x, :splatnew),
                       overdubbed_code) do x
            tagged_version = x.head == :new ? :tagged_new : :tagged_splatnew
            return Expr(:call, Expr(:nooverdub, GlobalRef(Cassette, tagged_version)),
                        overdub_ctx_slot, x.args...)
        end
    end
    ...
    if iskwfunc
        ...
        for i in 1:length(overdubbed_code)
            ...
            if Base.Meta.isexpr(stmt, :call)
                replacein[replaceat] = Expr(:call, GlobalRef(Cassette, replacewith),
                                            overdub_ctx_slot, stmt.args...)
            end
        end
    else
        ...
        stmtcount = (x, i) -> begin
            ...
            if Base.Meta.isexpr(stmt, :call) &&
               !(Base.Meta.isexpr(stmt.args[1], :nooverdub))
                isapplycall = is_ir_element(stmt.args[1], GlobalRef(Core, :_apply),
                                            overdubbed_code)
        ...
        newstmts = (x, i) -> begin
            ...
            isapplycall = is_ir_element(callstmt.args[1], GlobalRef(Core, :_apply),
                                        overdubbed_code)
            if isapplycall && arehooksenabled
                callf = callstmt.args[2]
                callargs = callstmt.args[3:end]
                stmts = Any[Expr(:call, GlobalRef(Core, :tuple), overdub_ctx_slot),
                            Expr(:call, GlobalRef(Core, :tuple), callf),
                            Expr(:call, GlobalRef(Core, :_apply),
                                 GlobalRef(Cassette, :prehook), SSAValue(i),
                                 SSAValue(i + 1), callargs...),
                            Expr(:call, GlobalRef(Core, :_apply),
                                 GlobalRef(Cassette, :overdub), SSAValue(i),
                                 SSAValue(i + 1), callargs...),
                            Expr(:call, GlobalRef(Core, :tuple), SSAValue(i + 3)),
                            Expr(:call, GlobalRef(Core, :_apply),
                                 GlobalRef(Cassette, :posthook), SSAValue(i),
                                 SSAValue(i + 4), SSAValue(i + 1), callargs...),
                            Base.Meta.isexpr(x, :(=)) ?
                            Expr(:(=), x.args[1], SSAValue(i + 3)) : SSAValue(i + 3)]
            elseif isapplycall
                callf = callstmt.args[2]
                callargs = callstmt.args[3:end]
                stmts = Any[Expr(:call, GlobalRef(Core, :tuple), overdub_ctx_slot, callf),
                            Expr(:call, GlobalRef(Core, :_apply),
            ...
            elseif arehooksenabled
                stmts = Any[Expr(:call, GlobalRef(Cassette, :prehook), overdub_ctx_slot,
                                 callstmt.args...),
                            Expr(:call, GlobalRef(Cassette, :overdub), overdub_ctx_slot,
                                 callstmt.args...),
                            Expr(:call, GlobalRef(Cassette, :posthook), overdub_ctx_slot,
                                 SSAValue(i + 1), callstmt.args...),
                            Base.Meta.isexpr(x, :(=)) ?
                            Expr(:(=), x.args[1], SSAValue(i + 1)) : SSAValue(i + 1)]
            else
    ...
    replace_match!(x -> overdub_ctx_slot, x -> Base.Meta.isexpr(x, :contextslot),
                   overdubbed_code)
...
@eval function _overdub_fallback($OVERDUB_CONTEXT_NAME, $OVERDUB_ARGUMENTS_NAME...)
    return fallback($OVERDUB_CONTEXT_NAME, $OVERDUB_ARGUMENTS_NAME...)
end
...
function recurse(ctx::Context, ::typeof(Core._apply), f, args...)
    return Core._apply(recurse, (ctx, f), args...)
end
if VERSION >= v"1.4.0-DEV.304"
    function recurse(ctx::Context, ::typeof(Core._apply_iterate), f, args...)
        ...
        return Core._apply_iterate((args...) -> recurse(ctx, f, args...), new_args...)
    end
end

function overdub_definition(line, file)
    return quote
        function $Cassette.overdub($OVERDUB_CONTEXT_NAME::$Cassette.Context,
                                   $OVERDUB_ARGUMENTS_NAME...)
            $(Expr(:meta, :generated_only))
            return $(Expr(:meta, :generated,
                          Expr(:new, Core.GeneratedFunctionStub, :__overdub_generator__,
                               Any[:overdub, OVERDUB_CONTEXT_NAME, OVERDUB_ARGUMENTS_NAME],
                               Any[], line, QuoteNode(Symbol(file)), true)))
        end
        function $Cassette.recurse($OVERDUB_CONTEXT_NAME::$Cassette.Context,
                                   $OVERDUB_ARGUMENTS_NAME...)
            $(Expr(:meta, :generated_only))
            return $(Expr(:meta, :generated,
                          Expr(:new, Core.GeneratedFunctionStub, :__overdub_generator__,
